...
export default function TrendingMentionsPage() {
  ...
  // Helper function to safely convert traffic to number
  const getTrafficNumber = (traffic: string | number | null | undefined): number => {
    if (traffic === null || traffic === undefined) return 0;
    if (typeof traffic === 'number') return traffic;
    const cleaned = String(traffic).replace(/,/g, '');
    return parseInt(cleaned, 10) || 0;
  };

  // Helper function to safely convert growth rate to number
  const getGrowthRateNumber = (growthRate: string | number | null | undefined): number => {
    if (growthRate === null || growthRate === undefined) return 0;
    if (typeof growthRate === 'number') return growthRate;
    const cleaned = String(growthRate).replace('%', '');
  ...
  const filterHashtags = () => {
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        case 'traffic':
          return getTrafficNumber(b.traffic) - getTrafficNumber(a.traffic);
        case 'growth':
          return getGrowthRateNumber(b.trafficGrowthRate) - getGrowthRateNumber(a.trafficGrowthRate);
        case 'time':
    ...
    console.log('Filtered hashtags:', filtered.length);
  ...
  const formatTraffic = (traffic: string | number | null | undefined) => {
    const num = getTrafficNumber(traffic);
    if (num >= 1000000) {
  ...
  const getGrowthColor = (growthRate: string | number | null | undefined) => {
    const rate = getGrowthRateNumber(growthRate);
    ...
    <div className={styles.container}>
      ...
      <div className={styles.controls}>
        <div className={styles.searchSection}>
          <div className={styles.searchInputContainer}>
            ...
            <input
              type="text"
              placeholder="Search trending mentions..."
              value={searchTerm}
      ...
      <div className={styles.stats}>
        <div className={styles.statCard}>
          ...
          <div className={styles.statLabel}>Trending Mentions</div>
        </div>
        <div className={styles.statCard}>
          <div className={styles.statNumber}>
            {filteredHashtags.reduce((sum, hashtag) => 
              sum + getTrafficNumber(hashtag.traffic), 0
            ).toLocaleString()}
          </div>
        ...
        <div className={styles.statCard}>
          <div className={styles.statNumber}>
            {filteredHashtags.length > 0 
              ? (filteredHashtags.reduce((sum, hashtag) => 
                  sum + getGrowthRateNumber(hashtag.trafficGrowthRate), 0
              ...
              <div className={styles.keywordInfo}>
                ...
                <div className={styles.metrics}>
                  <div className={styles.metric}>
                    <BarChart3 size={14} />
                    <span>Traffic: {typeof hashtag.traffic === 'number' ? hashtag.traffic.toLocaleString() : String(hashtag.traffic || '0')}</span>
                  </div>
                  <div className={styles.metric}>
                    <TrendingUp size={14} />
                    <span style={{ color: getGrowthColor(hashtag.trafficGrowthRate) }}>
                      {typeof hashtag.trafficGrowthRate === 'number' 
                        ? `${hashtag.trafficGrowthRate}%` 
                        : String(hashtag.trafficGrowthRate || '0%')}
                    </span>
                  </div>
                  <div className={styles.metric}>
                    <Clock size={14} />
      ...
      {filteredHashtags.length === 0 && !loading && (
        <div className={styles.emptyState}>
          ...
          <h3>No trending mentions found</h3>
